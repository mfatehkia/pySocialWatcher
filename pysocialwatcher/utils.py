...
def add_mocked_column(dataframe):
    dataframe["mock_response"] = dataframe["response"].apply(lambda response: True if (constants.MOCK_RESPONSE_FIELD in response.decode()) else False)
    return dataframe
...
def trigger_request_process_and_return_response(rows_to_request):
    ...
    # Starting process
    list(map(lambda p: p.start(), list_process))
    # Stop process
    list(map(lambda p: p.join(), list_process))
    #Check for Exception
    list(map(lambda p: check_exception(p), list_process))
...
def save_skeleton_dataframe(dataframe, output_dir = ""):
    ...
    dataframe.to_csv(output_dir + constants.DATAFRAME_SKELETON_FILE_NAME, index=False)
...
def save_temporary_dataframe(dataframe, output_dir = ""):
    ...
    dataframe.to_csv(output_dir + constants.DATAFRAME_TEMPORARY_COLLECTION_FILE_NAME, index=False)
...
def save_after_collecting_dataframe(dataframe, output_dir = ""):
    ...
    dataframe.to_csv(output_dir + constants.DATAFRAME_AFTER_COLLECTION_FILE_NAME, index=False)
...
def save_after_collecting_dataframe_without_full_response(dataframe, output_dir = ""):
    ...
    dataframe.to_csv(output_dir + constants.DATAFRAME_AFTER_COLLECTION_FILE_NAME_WITHOUT_FULL_RESPONSE, index=False)
